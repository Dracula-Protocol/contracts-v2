...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (chainId == '1') {
    ...
    const YAxisAdapter = await deploy('YAxisAdapter', {
      from: deployer,
      log: true,
      contract: 'YAxisAdapter',
      args: [WETH, uniFactory, masterVampire.address]
    });

    if (YAxisAdapter.newlyDeployed) {
      ...
      for (let pool of pools[POOL_ID].victimPools) {
        await masterVampire.add(YAxisAdapter.address, pool.victimPID);
      ...
      pools[POOL_ID].deployedAdapter = YAxisAdapter.address;
